  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 ${isRTL ? "left-6" : "right-6"} z-50 w-16 h-16 bg-gradient-to-br from-green-500 to-green-600 text-white rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center justify-center group`}
      ...
      >
        {isOpen ? (
          ...
          <>
            ...
            {/* Pulse indicator */}
            <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full animate-pulse flex items-center justify-center text-[10px] font-bold">?</span>
          </>
        )}
      </button>
      ...
      {isOpen && (
        <div
          className={`fixed bottom-24 ${isRTL ? "left-4" : "right-4"} z-50 w-[calc(100vw-32px)] sm:w-[400px] bg-white rounded-2xl shadow-2xl overflow-hidden`}
